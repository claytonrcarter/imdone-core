...
Task.getTags = function(text) {
  ...
  text = " " + text; // FIXME:0 filty hack so tasks can start with a tag
...
Task.getContext = function(text) {
  ...
  text = " " + text; // FIXME:10 dirty hack so tasks can start with a context
...
//#ARCHIVE:30 This is where we should look for tags /\s\+(\w*)/gi; and context /\s\@(\w*)/gi; +feature
